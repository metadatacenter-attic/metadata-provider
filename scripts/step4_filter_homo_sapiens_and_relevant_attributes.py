...
OUTPUT_FILE = constants.NCBI_FILTER_HS_AND_ATTS_OUTPUT_FILE
RELEVANT_ATTS = constants.NCBI_FILTER_RELEVANT_ATTS_1
...
def main():
    ...
    if os.path.exists(OUTPUT_FILE):
        if not utils.confirm('The destination file already exist. Do you want to overwrite it [y/n]? '):
            execute = False
    if execute:
        filter_utils.filter_samples(INPUT_FILE, OUTPUT_FILE, True, True, RELEVANT_ATTS, 100000)
